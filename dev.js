...
(function () {
  ...
  function bindUploadForm() {
    ...
    uploadForm.addEventListener('submit', async e => {
      ...
      const id = 'g' + Date.now();
      // Game DNA
      const dnaSpeed    = parseInt(val('upload-dna-speed'))    || 0;
      const dnaStrategy = parseInt(val('upload-dna-strategy')) || 0;
      const dnaSkill    = parseInt(val('upload-dna-skill'))    || 0;
      const dnaFun      = parseInt(val('upload-dna-fun'))      || 0;
      const dna = (dnaSpeed||dnaStrategy||dnaSkill||dnaFun)
        ? { speed: Math.min(100,Math.max(0,dnaSpeed)), strategy: Math.min(100,Math.max(0,dnaStrategy)), skill: Math.min(100,Math.max(0,dnaSkill)), fun: Math.min(100,Math.max(0,dnaFun)) }
        : null;
