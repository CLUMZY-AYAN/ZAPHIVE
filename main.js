...
(function () {
  ...
  function getGameTags(game) {
    ...
    // HOT: manual override OR auto (top 3 plays)
    if (mt.hot) {
      tags.push('hot');
    } else {
    ...
    // LIKES: manual override OR auto (top 3 likes)
    if (mt.likes) {
      tags.push('likes');
    } else {
    ...
    // NEW: manual override OR auto (added within 7 days)
    if (mt.new) {
      tags.push('new');
    } else if (game.dateAdded && n - new Date(game.dateAdded).getTime() < ONE_WEEK) {
      tags.push('new');
    }

    // UPDATED: manual override OR auto (embed changed within 3 days)
    if (mt.updated) {
      tags.push('updated');
    } else if (game.lastUpdated && n - new Date(game.lastUpdated).getTime() < THREE_DAYS) {
      tags.push('updated');
    }
