...
(function () {
  ...
  async function viewPlayerProfile(uid) {
    ...
    modal.innerHTML = `
      <div style="background:var(--bg-card);border:1px solid var(--accent-blue);border-radius:12px;padding:1.5rem;max-width:320px;width:100%;max-height:85vh;overflow-y:auto">
        ...
        ${user && user.role === 'dev' && uid !== user.uid ? `
        <!-- Dev role assignment -->
        <div style="margin-top:0.8rem;padding-top:0.8rem;border-top:1px solid var(--border)">
          <div style="font-size:0.68rem;color:var(--accent-yellow);font-weight:800;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:0.5rem">⚡ DEV: Assign Role</div>
          <div style="display:flex;gap:0.4rem">
            <select id="role-assign-select-${uid}" style="flex:1;background:rgba(26,140,255,0.06);border:1px solid var(--border);border-radius:6px;padding:0.4rem 0.6rem;color:var(--text-white);font-size:0.78rem">
              <option value="user">User</option>
              <option value="mod">Mod</option>
              <option value="admin">Admin</option>
            </select>
            <button onclick="window.ZhAssignRole('${uid}','${esc(p.username)}')" style="background:var(--accent-yellow);border:none;border-radius:6px;color:#000;font-size:0.75rem;font-weight:800;padding:0.4rem 0.8rem;cursor:pointer;font-family:var(--font-display)">Assign</button>
          </div>
          <div id="role-assign-result-${uid}" style="font-size:0.7rem;margin-top:0.35rem;min-height:1em"></div>
        </div>` : ''}
      </div>`;
  ...
  // ── Dev: Assign Role ─────────────────────────
  window.ZhAssignRole = async function(targetUid, targetUsername) {
    const user = getUser();
    if (!user || user.role !== 'dev') return;
    const sel = document.getElementById('role-assign-select-' + targetUid);
    const resultEl = document.getElementById('role-assign-result-' + targetUid);
    if (!sel || !resultEl) return;
    const newRole = sel.value;
    resultEl.style.color = 'var(--text-muted)';
    resultEl.textContent = 'Assigning…';
    try {
      await dbSet('/users/' + targetUid + '/role', newRole);
      // Log transaction
      const txId = 'tx_' + Date.now();
      await dbSet('/transactions/' + txId, {
        transactionId: txId,
        userId: targetUid,
        givenBy: user.uid,
        type: 'role_change',
        items: { role: newRole },
        reason: 'Role assigned by dev via profile',
        timestamp: Date.now()
      });
      resultEl.style.color = '#00c853';
      resultEl.textContent = '✓ ' + targetUsername + ' is now ' + newRole + '!';
    } catch(e) {
      resultEl.style.color = '#ff4d6d';
      resultEl.textContent = '✗ Failed. Check connection.';
    }
  };
